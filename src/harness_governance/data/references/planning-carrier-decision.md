...
1. 现有 project queue 优先于新建 planning files。
2. Change packet 承载跨层上下文，但不批准 implementation。
3. Execution prompt pack 只在计划或 packet 已批准后出现。
4. Task packet 只服务 implementation 执行，不替代 readiness 或 Implementation Entry Record。
5. 如果两个 carrier 都适用，选择能成为后续唯一 source of truth 的那个，并在另一个位置只放链接。

## Verification

选择完成后检查：

- 是否只有一个主 source of truth。
- 是否记录当前 harness layer。
- 是否有下一层候选或 stop condition。
- 是否没有采用外部 workflow 的默认 artifact 路径。
