...
_TIERS_JSON_TEMPLATE = """\
{
  ...
  "adapters": [
    ...
    },
    {
      ...
      "adapter": "subagent",
      "model_label": "<your-execution-model>"
    },
    {
      "role": "document-gardener",
      "required_tier": "mechanical",
      "adapter": "subagent",
      "model_label": "<your-light-model>"
    }
  ]
}
"""
...
# Map detected platform to its tiers.json directory.
_PLATFORM_TIERS_DIR: dict[str, Path] = {
...
def write_all_skill_files(
    project_root: Path, platform: str, tier_filter: str | None = None
) -> list[Path]:
    """Write skill files for *platform*, optionally restricted to *tier_filter*.

    When *tier_filter* is set, only that tier's skill file is written.
    When None, all applicable tiers are written.
    ...
    if tier_filter:
        tiers_to_write = [tier_filter]
    elif platform in _shared_file_platforms:
        tiers_to_write = ["standard"]
    else:
        tiers_to_write = list(GOVERNANCE_TIERS)
...
@click.option(
    "--tier",
    "tier",
    type=click.Choice(["light", "standard", "strict", "monitor"]),
    default=None,
    help="Governance tier for skill files (default: write all tiers).",
)
@click.pass_context
def init_cmd(
    ctx: click.Context,
    ...
    if all_platforms:
        ...
        tiers_to_write = [tier] if tier else list(GOVERNANCE_TIERS)
        seen_paths: set[Path] = set()
        for plat in sorted(PLATFORM_SKILL_PATHS):
            for t in tiers_to_write:
                tier_paths = PLATFORM_SKILL_PATHS_BY_TIER.get(
                    plat, PLATFORM_SKILL_PATHS_BY_TIER["generic"]
                )
                target = (project_root / tier_paths[t]).resolve()
                if target not in seen_paths:
                    seen_paths.add(target)
                    skill_paths.append(target)
            template_ver = extract_skill_version(
                load_skill_template(plat, tier or "standard")
            )
            ref_tier = tier or "standard"
            ref_target = (
                project_root
            ...
            if ref_target.exists() and not force:
                ...
                for t in tiers_to_write:
                    ...
                    tier_target = (project_root / tier_paths[t]).resolve()
                    if not tier_target.exists() or force:
                        write_skill_file(project_root, plat, t)
                        notes.append(
        ...
        if existing.exists() and not force:
            template_ver = extract_skill_version(
                load_skill_template(detected, tier or "standard")
            )
        ...
        if should_write:
            # Write tier-specific or all 4 tier skills
            written = write_all_skill_files(project_root, detected, tier_filter=tier)
    ...
    if not minimal:
        ...
        verification_dir = (project_root / "docs" / "verification").resolve()
        verification_dir.mkdir(parents=True, exist_ok=True)
        evidence_template = verification_dir / "user-evidence-template.md"
        if not evidence_template.exists():
            write_text_no_bom(evidence_template, _USER_EVIDENCE_TEMPLATE)
            notes.append(f"Created {evidence_template}")

        tests_dir = (project_root / "tests").resolve()
        tests_dir.mkdir(parents=True, exist_ok=True)
        state_contract_scaffold = tests_dir / "test_state_contract_scaffold.py"
        if not state_contract_scaffold.exists():
            write_text_no_bom(
                state_contract_scaffold,
                _STATE_CONTRACT_TEST_SCAFFOLD,
            )
            notes.append(f"Created {state_contract_scaffold}")

        # --- tiers.json: capability-tier subagent routing (v0.9.0) ---
        tiers_dir = (
            _PLATFORM_TIERS_DIR.get(detected, _PLATFORM_TIERS_DIR["generic"])
            if not all_platforms
            else Path(".agents")
        )
        tiers_path = (project_root / tiers_dir / "tiers.json").resolve()
        if not tiers_path.exists():
            tiers_path.parent.mkdir(parents=True, exist_ok=True)
            write_text_no_bom(tiers_path, _TIERS_JSON_TEMPLATE)
            notes.append(f"Created {tiers_path}")
