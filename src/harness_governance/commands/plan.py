...
def plan_init_cmd(ctx: click.Context, slug: str | None, template: str) -> None:
    ...
    if ctx.obj.get("json_output"):
        ...
        return
    click.echo(bilingual("plan.initialized", plan_id=session.plan_id))
    click.echo(bilingual("plan.dir", path=str(session.plan_dir)))
...
def plan_attest_cmd(ctx: click.Context, plan_id: str | None) -> None:
    ...
    try:
        ...
        raise click.ClickException(str(exc)) from exc
    short = digest[:12]
    click.echo(bilingual("plan.attest_locked", short=short))
...
def plan_show_cmd(ctx: click.Context, plan_id: str | None) -> None:
    ...
    click.echo(
        bilingual(
            "plan.show_header",
            plan_path=str(target.plan_dir / "task_plan.md"),
...
def plan_clear_cmd(ctx: click.Context, plan_id: str | None) -> None:
    ...
    if not attestation.is_file():
        ...
        return
    attestation.unlink()
    click.echo(bilingual("plan.cleared", plan_id=target.plan_id))
...
def plan_complete_cmd(ctx: click.Context, plan_id: str | None) -> None:
    ...
    try:
        ...
        raise click.ClickException(str(exc)) from exc
    if all_done:
        click.echo(bilingual("plan.all_complete"))
        raise click.exceptions.Exit(code=0)
    click.echo(bilingual("plan.incomplete"))
