...
**优先级判断**：
P0 第一版已实现。原因是它约束的是“测试证据是否能证明用户可感知功能真的可交付”，属于交付可靠性硬门禁，而不是 P1 增强项。State Contract 解决治理状态闭环；User-Perceived Integration Evidence Gate 解决 agent 用 smoke/contract/自造数据冒充真实用户闭环的问题。第一版先做文档级机器检查；后续可逐步增强到 AST / Playwright trace / request payload / selector 扫描。
