...
### 3E. User-Perceived Integration Evidence Gate / 用户感知集成证据门禁（新增，P1）
...
**实现位置**：
- `tests/VERIFICATION_CONTRACTS.md`：新增验证证据契约。
- `src/harness_governance/commands/check.py`：新增 `harness check user-evidence` 并接入 `check all`。
- `src/harness_governance/commands/aliases.py`：`harness ship` 纳入 user-evidence 检查。
- `src/harness_governance/commands/init.py`：生成 `docs/verification/user-evidence-template.md` 或派生项目测试/验证骨架。
- `src/harness_governance/state_machine/gates.py`：在 readiness / verification 层加入用户感知证据问题和门禁要求。
- `tests/test_commands/test_check_cmd.py`：覆盖缺字段、自证词、Not Applicable、MVP 命名偷换、通过样例和 `check all` 集成。
- 反例 fixture：加入类似 `web-designer-real-save-loop` 的 negative case，证明旧规则会误放行、新规则会阻止。

**优先级判断**：
P1，且应高于 P2。原因是它约束的是“测试证据是否能证明用户可感知功能真的可交付”，属于交付可靠性硬门禁。State Contract 解决治理状态闭环；User-Perceived Integration Evidence Gate 解决 agent 用 smoke/contract/自造数据冒充真实用户闭环的问题。第一版可先做文档级机器检查，再逐步增强到 AST / Playwright trace / request payload / selector 扫描。
...
| **P1** | User-Perceived Integration Evidence Gate / 用户感知集成证据门禁 | Dogfood | 中 | 待实现 — 强制 real-user acceptance 证据，防止 smoke/contract 冒充 MVP 闭环 |
...
3. **P1 新增**：实现 `harness check user-evidence`，强制用户感知集成证据字段、证据分级和 anti-self-proof 断言。
4. **P1 增强**：将 `user-evidence` 接入 `harness check all` / `harness ship` / verification gate。
5. **P1 增强**：在 `harness init` 中生成派生项目 user-evidence 模板。
6. **P1 增强**：在 `harness ship` 输出中提示 tag release 前安装/运行 release 验证（仅本仓库）。
7. **P2 设计**：定义 `spec quick` 与完整 change packet 的升级边界。
